...
Imprimir(PROW()+1,10,RUC)
Imprimir(PROW()+1,20,Cliente)
Imprimir(PROW()+1,15,Direccion)
...
m.Precio = NVL(real,precio)
	Imprimir(PROW()+2,5,IdProducto,'XXXXXXXXXX')
	Imprimir(PROW(),15,Cantidad,'999999999')
	Imprimir(PROW(),30,Producto,REPLICATE('X',40))
	Imprimir(PROW(),70,m.Precio,cMascara)
	
	IF Iva=0
		Imprimir(PROW(),87,m.Precio*Cantidad,cMascara)
		mIva0=mIva0 + m.Precio*Cantidad	
	ENDIF
	
	IF Iva=5
		Imprimir(PROW(),100,m.Precio*Cantidad,cMascara)
		mIva5=mIva5 + m.Precio*Cantidad	
	ENDIF
	
	IF Iva=10
		Imprimir(PROW(),115,m.Precio*Cantidad,cMascara)
		 mIva10=mIva10 + m.Precio*Cantidad	
	ENDIF
ENDSCAN


Imprimir(fpos + 22,90,mIva0,cMascara)
Imprimir(PROW(),100,m.mIva5,cMascara)
Imprimir(PROW(),115,m.mIva10,cMascara)

Imprimir(PROW()+1,25,LEFT(m.mletras,75))
Imprimir(PROW(),115,mTotal,cMascara)
Imprimir(PROW()+1,20,substr(m.mletras,76,75))

Imprimir(PROW()+1,75,round(mIva5*5/105,m.dec),cMascara)
